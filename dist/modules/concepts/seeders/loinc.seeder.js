...
let LoincSeederService = LoincSeederService_1 = class LoincSeederService {
    ...
    async seedLoincData(triggeredBy = 'system') {
        ...
        try {
            ...
            const { codesCreated, valuesCreated, rowsProcessed } = await this.processChangedRows(currentData.rows, changedRows, stats, currentData.headers);
    ...
    async processChangedRows(allRows, changedRows, stats, headers) {
        let codesCreated = 0;
        let valuesCreated = 0;
    ...
    async getChangedRowsFromPreviousImport(previousTracking) {
        return {
            added: [],
            modified: [],
            deleted: [],
